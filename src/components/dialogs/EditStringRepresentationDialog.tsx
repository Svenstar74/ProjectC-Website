...
import { updateClimateConceptNodeStringRepresentation } from '../../store/redux/slices/dataSlice';
...
function EditStringRepresentationDialog({
  ...
  const submitForm = async () => {
    ...
    const result = await apiClient.updateStringRepresentation(climateConceptId, stringRepresentation);
    if (result === 200) {
      dispatch(updateClimateConceptNodeStringRepresentation({ id: climateConceptId, newName: stringRepresentation}));
      globalSigmaInstance?.getGraph().setNodeAttribute(climateConceptId, 'label', stringRepresentation);
      onClose();
    }
  };
